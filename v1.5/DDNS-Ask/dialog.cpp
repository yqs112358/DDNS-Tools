...
void Dialog::init()
{
    ...
    {
        QHostInfo info=QHostInfo::fromName(ui->lineEdit_server->text());
        if(info.error() != QHostInfo::NoError || info.addresses().isEmpty())
        {
            QMessageBox::warning(this,tr("错误"),tr("无法查询到输入的服务器地址！\n")+info.errorString(),QMessageBox::Ok,QMessageBox::Ok);
            setMsg(tr("【状态】等待查询。"));
            enableInput();
            return;
        }
